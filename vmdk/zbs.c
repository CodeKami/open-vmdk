...
const char* ZBS_HOST = "10.23.4.43:10201:10206";
const char* VOLUME_ID = "a7581393-547b-4cd0-a9af-b1b9a7881ec7";
...
ssize_t zbs_pread_volume(zbs* z, void* buf, size_t n_bytes, off_t offset) {
  int read_ret;
  read_ret = zbs_read(z->zbs_client, z->zbs_volume_id, buf, (uint32_t)n_bytes,
                      (uint64_t)offset);
  if (read_ret < 0) {
...
ssize_t zbs_pwrite_volume(zbs* z, void* buf, size_t n_bytes, off_t offset) {
  ...
  write_ret = zbs_write(z->zbs_client, z->zbs_volume_id, buf, (uint32_t)n_bytes,
                        (uint64_t)offset);
  if (write_ret < 0) {
...
ssize_t zbs_get_size(zbs* z) { return z->seek; }
